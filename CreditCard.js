function isValidCard() { //creo una función para validar mi tarjeta de credito
    ...
    var creditCard = arregloDeSubCadenas; //creditCard será igual al resultado de arregloDeSubCadenas
    ...
    for (var i = 0; i < creditCard.length; i++) { //con el ciclo for recorro los 16 dígitos de la tarjeta de credito para averiguar si es válida según al algoritmo de Luhn
        ...
        if (i % 2 == 1) { //si el residuo de i entre dos es 1 significará que el indice de la posición es impar(ya que en el ejemplo de Michele las posiciones comenzaban en el 1 y en mi array comienzan en el 0)
        ...
        numLuhn += value; //finalmente a la variable numLuhn le sumamos el valor de cada uno de los digitos y nuevos digitos en cada uno de los 16 ciclos.
    }
    ...
    if (result == true) { //valido si numLuhn es 0 o no y escribo en pantalla el mensaje
        ...
        document.write /*alert*/ ("<h1>¡La tarjeta " + creditCardNumber + " es válida! :D </h1>"); //Si la tarjeta es válida (con el número original ingresado por el usuario)

    } else if (result == false) {

        document.write /*alert*/ ("<h1>¡La tarjeta " + creditCardNumber + " es inválida! :'( </h1>"); //Si la tarjeta es inválida (con el número original ingresado por el usuario)
